...
class DB
{
    ...
    public function getLatestPostDetail()
    {
        $sql = "SELECT * FROM posts ORDER BY created_at DESC LIMIT 0, 1";
        $stm = $this->connection->prepare($sql);
        $stm->execute();
        $result = $stm->fetch(\PDO::FETCH_ASSOC);
    ...
    public function insertPostComment($postId, $userName, $content)
    {
        ...
        $sql = "INSERT INTO comments(content, created_at, posts_id, users_id) VALUES(:content, '".$dateTime."', :post_id, :user_id)";
        $stm = $this->connection->prepare($sql);
        $stm->bindValue(':content', $content);
        $stm->bindValue(':post_id', $postId);
